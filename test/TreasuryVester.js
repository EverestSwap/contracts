...
describe("TreasuryVester.sol", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy EVRS
    this.evrs = await this.Evrs.deploy(
    ...
    await this.evrs.setMinter(this.vester.address);
  ...
  describe("Constructor", function () {
    it("arg 1: vesting token", async function () {
      expect(await this.vester.vestedToken()).to.equal(this.evrs.address);
    });
  ...
  describe("distribute", function () {
    it("cannot distribute when vesting disabled", async function() {
      ...
      var oldSupply = await this.evrs.totalSupply();
      ...
      var newSupply = await this.evrs.totalSupply();
    ...
    it("cannot distribute before vesting cliff is over", async function() {
      ...
      var newerSupply = await this.evrs.totalSupply();
    ...
    it("distributes to MiniChef", async function() {
      ...
      chef = await this.Chef.deploy(this.evrs.address, this.admin.address);
      await chef.deployed();
      ...
      var chefBalance = await this.evrs.balanceOf(chef.address);
  ...
  describe("Semi-Random Vesting Simulation", function () {
    it("sets up contract for vesting", async function () {
      ...
      this.chef = await this.Chef.deploy(this.evrs.address,this.admin.address);
    ...
    for (let month = 0; month < 30; month++) {
      it("vests month " + (month + 1), async function () {
        ...
        let balance = await this.evrs.totalSupply();
    ...
    it("fails vesting after 30th month", async function () {
      ...
      let balance = await this.evrs.balanceOf(this.vester.address);
    ...
    it("recipients got correct allocation", async function () {
      for (let i = 0; i < this.recipients.length; i++) {
        let balance = await this.evrs.balanceOf(this.recipients[i].account);
