...
describe('CommunityTreasury', function () {
    ...
    describe("Constructor", function () {
        it('EVRS', async function () {
            expect(await this.community.evrs()).to.equal(this.evrs.address);
        });
    ...
    describe("Transfer", function () {
        it('Owner can transfer', async function () {
            ...
            expect(await this.evrs.balanceOf(this.community.address)).to.equal(transferAmount);
            expect(await this.evrs.balanceOf(this.addr2.address)).to.equal(0);

            await this.community.transfer(this.addr2.address, transferAmount);
            expect(await this.evrs.balanceOf(this.addr2.address)).to.equal(transferAmount);
        });
        ...
        it('No balance to transfer', async function () {
            const transferAmount = oneToken.mul(100);
            expect(await this.evrs.balanceOf(this.community.address)).to.equal(0);

            await expect(this.community.transfer(this.addr2.address, transferAmount)).to.be.revertedWith(
                'Evrs::_transferTokens: transfer amount exceeds balance'
            );
        });
    });
