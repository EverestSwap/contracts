...
contract EverestBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'EverestBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'EverestBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "EverestBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "EverestBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "EverestBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "EverestBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IEverestERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IEverestPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = EverestLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = EverestLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IEverestPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IEverestPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "EverestBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "EverestBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "EverestBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IEverestPair(pairA).token0() == IEverestPair(pairB).token0() ||
            IEverestPair(pairA).token0() == IEverestPair(pairB).token1() ||
            IEverestPair(pairA).token1() == IEverestPair(pairB).token0() ||
            IEverestPair(pairA).token1() == IEverestPair(pairB).token1(),
            "EverestBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "EverestBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "EverestBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IEverestPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IEverestPair(liquidityPairFrom).token0();
        if (
            IEverestPair(liquidityPairFrom).token0() == IEverestPair(liquidityPairTo).token0() ||
            IEverestPair(liquidityPairFrom).token0() == IEverestPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IEverestPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "EverestBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IEverestPair(liquidityPairTo).token0() ||
            newTokenAddress == IEverestPair(liquidityPairTo).token1(),
            "EverestBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IEverestPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IEverestPair(liquidityPairFrom).token0() != IEverestPair(liquidityPairTo).token0() &&
            IEverestPair(liquidityPairFrom).token1() != IEverestPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IEverestPair(liquidityPairTo).token0(), IEverestPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IEverestPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IEverestPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IEverestPair(pairAddress).getReserves();
        uint totalSupply = IEverestPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "EverestBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "EverestBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IEverestPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = EverestLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
