...
import './interfaces/IWICY.sol';

contract EverestRouter is IEverestRouter {
    using SafeMath for uint;

    address public immutable override factory;
    address public immutable override WICY;
    ...
    constructor(address _factory, address _WICY) public {
        factory = _factory;
        WICY = _WICY;
    }

    receive() external payable {
        assert(msg.sender == WICY); // only accept ICY via fallback from the WICY contract
    }
    ...
    function addLiquidityICY(
        ...
        address to,
        uint deadline
    ) external virtual override payable ensure(deadline) returns (uint amountToken, uint amountICY, uint liquidity) {
        (amountToken, amountICY) = _addLiquidity(
            token,
            WICY,
            amountTokenDesired,
            msg.value,
        ...
        address pair = EverestLibrary.pairFor(factory, token, WICY);
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWICY(WICY).deposit{value: amountICY}();
        assert(IWICY(WICY).transfer(pair, amountICY));
        liquidity = IEverestPair(pair).mint(to);
        // refund dust ICY, if any
        if (msg.value > amountICY) TransferHelper.safeTransferICY(msg.sender, msg.value - amountICY);
    }
    ...
    function removeLiquidityICY(
        ...
        address to,
        uint deadline
    ) public virtual override ensure(deadline) returns (uint amountToken, uint amountICY) {
        (amountToken, amountICY) = removeLiquidity(
            token,
            WICY,
            liquidity,
            amountTokenMin,
        ...
        IWICY(WICY).withdraw(amountICY);
        TransferHelper.safeTransferICY(to, amountICY);
    }
    ...
    function removeLiquidityICYWithPermit(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external virtual override returns (uint amountToken, uint amountICY) {
        address pair = EverestLibrary.pairFor(factory, token, WICY);
        uint value = approveMax ? uint(-1) : liquidity;
        IEverestPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        (amountToken, amountICY) = removeLiquidityICY(token, liquidity, amountTokenMin, amountICYMin, to, deadline);
    }
    ...
    function removeLiquidityICYSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) public virtual override ensure(deadline) returns (uint amountICY) {
        (, amountICY) = removeLiquidity(
            token,
            WICY,
            liquidity,
            amountTokenMin,
    ...
    function removeLiquidityICYWithPermitSupportingFeeOnTransferTokens(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external virtual override returns (uint amountICY) {
        address pair = EverestLibrary.pairFor(factory, token, WICY);
        uint value = approveMax ? uint(-1) : liquidity;
        IEverestPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        amountICY = removeLiquidityICYSupportingFeeOnTransferTokens(
            token, liquidity, amountTokenMin, amountICYMin, to, deadline
        );
    }
    ...
    {
        require(path[0] == WICY, 'EverestRouter: INVALID_PATH');
        amounts = EverestLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'EverestRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWICY(WICY).deposit{value: amounts[0]}();
        assert(IWICY(WICY).transfer(EverestLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactICY(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WICY, 'EverestRouter: INVALID_PATH');
        amounts = EverestLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'EverestRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, EverestLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        IWICY(WICY).withdraw(amounts[amounts.length - 1]);
        TransferHelper.safeTransferICY(to, amounts[amounts.length - 1]);
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        _swap(amounts, path, to);
        // refund dust ICY, if any
        if (msg.value > amounts[0]) TransferHelper.safeTransferICY(msg.sender, msg.value - amounts[0]);
    }
        ...
        ensure(deadline)
    {
        ...
        IWICY(WICY).deposit{value: amountIn}();
        assert(IWICY(WICY).transfer(EverestLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IERC20(WICY).balanceOf(address(this));
        require(amountOut >= amountOutMin, 'EverestRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWICY(WICY).withdraw(amountOut);
        TransferHelper.safeTransferICY(to, amountOut);
    }
