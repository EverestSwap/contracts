...
contract EverestFeeCollector is Ownable {

    using SafeERC20 for IERC20;
    using Address for address;

    address public immutable everestRouter;
    ...
    function _pullLiquidity(IEverestPair pair, uint256 balance) internal {
        pair.approve(everestRouter, balance);
        IEverestRouter(everestRouter).removeLiquidity(
            pair.token0(),
            pair.token1(),
            balance,
            0,
    ...
    internal {
        ...
        IERC20(token).safeApprove(everestRouter, 0);
        IERC20(token).safeApprove(everestRouter, amount);
        IEverestRouter(everestRouter).swapExactTokensForTokens(
            amount,
            0,
            path,
            address(this),
    ...
    function _collectFees(address[] memory liquidityPairs,
        ...
        for (uint256 i; i < liquidityPairs.length; ++i) {
            ...
            IEverestPair currentPair = IEverestPair(currentPairAddress);
            uint256 evrslBalance = currentPair.balanceOf(address(this));
            if (evrslBalance > 0) {
                _pullLiquidity(currentPair, evrslBalance);
