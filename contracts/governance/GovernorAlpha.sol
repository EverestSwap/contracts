...
contract GovernorAlpha {
    ...
    string public constant name = "Everest Governor Alpha";
    ...
    /// @notice The address of the Everest Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the Everest governance token
    EvrsInterface public evrs;
    ...
    constructor(address timelock_, address evrs_, address guardian_, uint threshold_) public {
        timelock = TimelockInterface(timelock_);
        evrs = EvrsInterface(evrs_);
        guardian = guardian_;
        proposalThreshold = threshold_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(evrs.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold, "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(evrs.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold, "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = evrs.getPriorVotes(voter, proposal.startBlock);
