...
// produces names for pairs of tokens using Everest's naming scheme
library PairNamer {
    string private constant TOKEN_SYMBOL_PREFIX = 'EVRS:';
    ...
    // produces a pair symbol in the format of `EVRS:${symbol0}:${symbol1}${suffix}`
    function pairSymbol(
        address token0,
        address token1,
        string memory suffix
